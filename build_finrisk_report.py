...
story += [P("4. Model training and evaluation", "H1X"),
          ...
          P("A compact Random Forest Regressor is the single loan-amount model. The project ships exactly one model per task - Gradient Boosting for the risk tier and the Random Forest Regressor for the amount - so there is no model-selection step to maintain. The regressor is trained with a monotonic constraint on credit score and credit-health score, so a stronger applicant is never recommended a smaller loan.", "BodyX"),
